...
public class JsonParseTemplate : MonoBehaviour {
    ...
    {
        /* ----- 通过Resources.load()获取Resources下的文件，TextAsset也可是直接读取json文件 ,android是可以的 ----- */
        // 获取文件的Resources下路径
        string rewardBubblePath = ConstTemplate.resRewardBubbleLevelPath + ConstTemplate.resRewardBubbleLevelName + string.Format("{0:D2}", levelId);
        TextAsset jsonAsset = Resources.Load(rewardBubblePath) as TextAsset;//本地
        if (jsonAsset == null)
            Debug.LogError("-- silent -- " + rewardBubblePath + "  no exsit");
        // 返回解析json的数据结构
        return JsonUtility.FromJson<RewardBubbleLevelData>(jsonAsset.text);


        /* ----- 绝对路径获取的，但是在android上是不可以的 ----- */
        //// 获取文件的绝对路径
    ...
    public static GermLevelData LoadGermLevelJsonData(int levelId)
    {
        /* ----- 通过Resources.load()获取Resources下的文件，TextAsset也可是直接读取json文件 ,android是可以的 ----- */
        // 获取文件的Resources下路径
        string germPath = ConstTemplate.resGermLevelPath + ConstTemplate.resGermLevelName + string.Format("{0:D2}", levelId);
        TextAsset jsonAsset = Resources.Load(germPath) as TextAsset;//本地
        if (jsonAsset == null)
            Debug.LogError("-- silent -- " + germPath + "  no exsit");
        // 返回解析json的数据结构
        return JsonUtility.FromJson<GermLevelData>(jsonAsset.text);
    ...
    public static RewardToolsLevelData LoadRewardToolLevelJsonData(int levelId)
    {
        ...
        /* ----- 通过Resources.load()获取Resources下的文件，TextAsset也可是直接读取json文件 ,android是可以的 ----- */
        // 获取文件的Resources下路径
        string rewardToolPath = ConstTemplate.resRewardToolLevelPath + ConstTemplate.resRewardToolLevelName + string.Format("{0:D2}", levelId);
        TextAsset jsonAsset = Resources.Load(rewardToolPath) as TextAsset;//本地
        if (jsonAsset == null)
            Debug.LogError("-- silent -- " + rewardToolPath + "  no exsit");
        // 返回解析json的数据结构
        return JsonUtility.FromJson<RewardToolsLevelData>(jsonAsset.text);
